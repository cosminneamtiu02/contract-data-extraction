...
- Phase 0.5 CI/CD design + accepted deviations log: [docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md](docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md) (§17 latest: §17.39 — single-cycle standalone review on `chore/panel-review-fixes-2026-05-13-pass3` (opus, first run under EXHAUSTIVE PASS RULE); 13 fix-commits + 1 audit commit = 14 commits in one pass; severity-by-finding tally (post-filter): 1 Crit / 9 Imp / ~10 Min; raw volume ~127 vs prior cycles' ~30–60 — filter handled 80% drop as designed; 1 multi-lens convergence (SIDE_CHANNEL_KEYS L07+L13, headbutting resolved keep+test); pre-commit hook race during 8-agent parallel dispatch produced 3 commit-title-vs-content mismatches + 1 duplicate title — recovery commit added for orphaned dependabot.yml work; single-cycle TERMINAL per user "run that once" invocation) — see §17.39 for proposed Workflow-Gap Rule #5 (parallel-dispatch commit-attribution race) + restated Rule #4 (pre-audit grep checklist).
