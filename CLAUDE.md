...
- Phase 0.5 CI/CD design + accepted deviations log: [docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md](docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md) (§17 latest: §17.40 — Cycle 2 of auto-converge loop on `chore/panel-review-fixes-2026-05-13-pass3` (opus); 12 fix-commits in cycle 1 + 5 fix-commits in cycle 2 + 2 audit commits = 19 total branch commits across 2 cycles; cycle-2 severity-by-finding tally (post-filter): 0 Crit / 3 Imp / 6 Min; cycle-2 raw volume ~132 (near steady-state vs cycle-1 ~127); 1 multi-lens convergence (L01+L17 stale §17-latest pointers); 0 user-decision items (loop mode auto-decides; cycle-1's 3 deferrals stand); forward-pointer index for the 3 §17.39 commit-title-vs-content mismatches added in §17.40 instead of per-commit edits (immutable on branch); Workflow-Gap Rule #3 (audit-comment factual drift) and Rule #2 (pointer leaks) both fired again — Rule #4 (pre-audit grep checklist) still pending in CLAUDE.md) — see §17.40 for the cycle-2 status line and forward-pointer index.
...
- Phase 3 LLM-layer spec + accepted deviations log: [docs/superpowers/specs/2026-05-13-phase-3-llm-spec-deviations.md](docs/superpowers/specs/2026-05-13-phase-3-llm-spec-deviations.md) (own §17 namespace, distinct from the CI/CD and Phase 2 specs — qualify cross-file references with the filename to disambiguate; latest: §17.5 — PR #14 squash subject under-stated material constituents (`SIDE_CHANNEL_KEYS` public export + `ContextOverflowError` mapping + dev-mode `_debug` block / `ClientMode` literal alias) — cross-reference audit trail so future grep against the squash subject can find the under-stated surface)
