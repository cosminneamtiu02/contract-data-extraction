...
| 1.9 | Structlog config | `src/extraction_service/log_config.py` (renamed from `logging.py` to avoid stdlib shadowing) | `test_configure_logging_production_emits_json_*`; `test_configure_logging_dev_emits_human_readable_not_json` (in `tests/unit/test_log_config.py`) | configure_logging(mode) sets renderers | pytest |
