...
> The block below is the original plan-time snapshot. The live `pyproject.toml` is the source of truth — it has diverged since Phase 1 review passes added ruff rule-set entries (`EM`, `TRY`, `TC` — note: this rule family was originally added under the legacy `TCH` selector name and was renamed to `TC` per ruff 0.15.x's category-prefix deprecation; see `docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md §17.39`), `N`, `S`, per-file-ignores for tests, pytest `markers`/`filterwarnings`, the hatchling `exclude` directive, and version floors on the `types-*` dev deps. See `pyproject.toml` directly for current state; deviations from this snapshot are recorded in `docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md §17.8`.
...
|---|---|---|---|---|---|
| 1.1 | `ContractJob` frozen Pydantic model | `src/extraction_service/domain/job.py` | `test_contract_job_metadata_defaults_to_empty_dict`; `test_contract_job_is_frozen`; `test_contract_job_round_trips_through_model_dump_json`; `test_contract_job_raises_when_both_required_fields_missing`; `test_contract_job_raises_when_pdf_bytes_missing`; `test_contract_job_raises_when_contract_id_missing`; `test_contract_job_rejects_unknown_field` (in `tests/unit/test_domain_job.py`) | Pydantic v2 model with `model_config = ConfigDict(frozen=True, extra="forbid")`, fields: `contract_id: UUID`, `pdf_bytes: bytes`, `metadata: dict[str, Any]`. `extra="forbid"` added via panel-review-fix on `chore/panel-review-fixes-2026-05-13-pass3` (mirrors the cycle-1 ContractRecord decision; see `docs/superpowers/specs/2026-05-11-ci-cd-scaffolding-design.md §17.40`) for typo-rejection at the Phase 4/5 intake boundary. | `uv run pytest tests/unit/test_domain_job.py` |
...
| # | Task | File | RED test | GREEN impl | Verify |
|---|---|---|---|---|---|
| 3.1 | Ollama client wrapper | `src/extraction_service/llm/client.py` | `test_ollama_client_emits_canonical_chat_call_shape` (use `FakeOllamaClient`; consolidated from `test_ollama_client_calls_correct_endpoint` + 3 sibling tautologies — see `docs/superpowers/specs/2026-05-13-phase-3-llm-spec-deviations.md §17.6`. Plan column lists only the seed RED test per task; additional behavior-coverage tests are tracked in the §17.6 entry of the Phase 3 spec.) | thin wrapper over `ollama.AsyncClient`, exposes `extract(prompt, schema) -> dict` | pytest |
