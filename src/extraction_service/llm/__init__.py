...
``SIDE_CHANNEL_KEYS`` is a frozenset Phase 4 callers iterate over to strip
metadata keys (today: ``{"_debug"}``) from a dev-mode ``extract`` result
before validation or HTTP serialization — see ``client.py`` for the full
rationale.
...
from extraction_service.llm.client import (
    SIDE_CHANNEL_KEYS,
    ClientMode,
    OllamaLlmClient,
)
